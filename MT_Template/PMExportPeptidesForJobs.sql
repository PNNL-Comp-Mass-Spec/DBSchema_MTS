/****** Object:  StoredProcedure [dbo].[PMExportPeptidesForJobs] ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE Procedure PMExportPeptidesForJobs
/****************************************************	
**  Desc:	
...
**
**  Auth:	mem
**	Date:	10/29/2009 mem - Initial Version (with most of the code coming from GetPeptidesAndSICStats)
**			11/11/2009 mem - Added parameter @ReturnJobInfoTable
...
AS
	...
	Begin Try
		...
		Begin -- <a1>
			...
			Set @S = @S +       ' Job, Peptide_ID, Scan_Number, Cleavage_State, RankHit, DelM_PPM, Mass_Tag_ID, Peptide, Monoisotopic_Mass,'
			...
			Set @S = @S + ' SELECT Pep.Job, Pep.Peptide_ID, Pep.Scan_Number, MT.Cleavage_State_Max, Pep.RankHit, Pep.DelM_PPM, '
			...
			If @Iteration = 5
				Set @S = @S +    ' M.Normalized_Score, M.PValue, M.FDR, 1 AS DelCN2, Pep.RankHit AS RankXC, '
		

			Set @S = @S +        ' Pep.Charge_State, SD.DiscriminantScoreNorm, '
			...
			If @MinimumPMTQualityScore > 0
				Set @S = @S +   ' MT.PMT_Quality_Score >= @MinimumPMTQualityScore AND '
						
			Set @S = @S +   '( '
			
			If @MinimumCleavageState < 2
				Set @S = @S +    '(MT.Cleavage_State_Max = 2 AND '
				
			If @Iteration = 1
			Begin
				Set @S = @S +    '(Pep.Charge_State = 1 AND SS.DeltaCn2 >= @MinDelCn2 AND SS.XCorr >= @MinXCorrFullyTrypticCharge1 OR'
				Set @S = @S +    ' Pep.Charge_State = 2 AND SS.DeltaCn2 >= @MinDelCn2 AND SS.XCorr >= @MinXCorrFullyTrypticCharge2 OR'
				Set @S = @S +    ' Pep.Charge_State >= 3 AND SS.DeltaCn2 >= @MinDelCn2 AND SS.XCorr >= @MinXCorrFullyTrypticCharge3)'
			End

			If @Iteration = 2
			Begin
				Set @S = @S +    '(Pep.Charge_State = 1 AND X.DeltaCn2 >= @MinDelCn2 AND X.Normalized_Score >= @MinXCorrFullyTrypticCharge1 OR'
				Set @S = @S +    ' Pep.Charge_State = 2 AND X.DeltaCn2 >= @MinDelCn2 AND X.Normalized_Score >= @MinXCorrFullyTrypticCharge2 OR'
				Set @S = @S +    ' Pep.Charge_State >= 3 AND X.DeltaCn2 >= @MinDelCn2 AND X.Normalized_Score >= @MinXCorrFullyTrypticCharge3)'
			End

			If @Iteration = 3
			Begin
				Set @S = @S +    '(Pep.Charge_State = 1 AND I.DeltaScore >= @MinDelCn2 AND I.Normalized_Score >= @MinXCorrFullyTrypticCharge1 OR'
				Set @S = @S +    ' Pep.Charge_State = 2 AND I.DeltaScore >= @MinDelCn2 AND I.Normalized_Score >= @MinXCorrFullyTrypticCharge2 OR'
				Set @S = @S +    ' Pep.Charge_State >= 3 AND I.DeltaScore >= @MinDelCn2 AND I.Normalized_Score >= @MinXCorrFullyTrypticCharge3)'
			End
			...
			If @MinimumCleavageState < 2
			Begin -- <b0>
				Set @S = @S +    ') OR (MT.Cleavage_State_Max < 2 AND '

				If @Iteration = 1
				Begin
					Set @S = @S +    '(Pep.Charge_State = 1 AND SS.DeltaCn2 >= @MinDelCn2 AND SS.XCorr >= @MinXCorrPartiallyTrypticCharge1 OR'
					Set @S = @S +    ' Pep.Charge_State = 2 AND SS.DeltaCn2 >= @MinDelCn2 AND SS.XCorr >= @MinXCorrPartiallyTrypticCharge2 OR'
					Set @S = @S +    ' Pep.Charge_State >= 3 AND SS.DeltaCn2 >= @MinDelCn2 AND SS.XCorr >= @MinXCorrPartiallyTrypticCharge3)'
				End

				If @Iteration = 2
				Begin
					Set @S = @S +    '(Pep.Charge_State = 1 AND X.DeltaCn2 >= @MinDelCn2 AND X.Normalized_Score >= @MinXCorrPartiallyTrypticCharge1 OR'
					Set @S = @S +    ' Pep.Charge_State = 2 AND X.DeltaCn2 >= @MinDelCn2 AND X.Normalized_Score >= @MinXCorrPartiallyTrypticCharge2 OR'
					Set @S = @S +    ' Pep.Charge_State >= 3 AND X.DeltaCn2 >= @MinDelCn2 AND X.Normalized_Score >= @MinXCorrPartiallyTrypticCharge3)'
				End

				If @Iteration = 3
				Begin
					Set @S = @S +    '(Pep.Charge_State = 1 AND I.DeltaScore >= @MinDelCn2 AND I.Normalized_Score >= @MinXCorrPartiallyTrypticCharge1 OR'
					Set @S = @S +    ' Pep.Charge_State = 2 AND I.DeltaScore >= @MinDelCn2 AND I.Normalized_Score >= @MinXCorrPartiallyTrypticCharge2 OR'
					Set @S = @S +    ' Pep.Charge_State >= 3 AND I.DeltaScore >= @MinDelCn2 AND I.Normalized_Score >= @MinXCorrPartiallyTrypticCharge3)'
				End
			...
			Set @S = ''
			Set @S = @S +   ' Job, Peptide_ID, Scan_Number, Cleavage_State, RankHit, DelM_PPM, Mass_Tag_ID, Peptide,'
		...
		Begin -- <a2>
			...
			If @Iteration = 4
			Begin
				Set @ResultType = 'MSG_Peptide_Hit'
				...
				Begin -- <c>
					...
					Begin
						if @PreviewSql <> 0
							...
							exec sp_ExecuteSql @InsertSqlPeptideHit, @Params, @MinimumCleavageState, @MinimumDiscriminantScore, @MinimumPeptideProphetProbability, @MSGFThreshold, @MinimumPMTQualityScore, @MinDelCn2, @MinXCorrFullyTrypticCharge1, @MinXCorrFullyTrypticCharge2, @MinXCorrFullyTrypticCharge3, @MinXCorrPartiallyTrypticCharge1, @MinXCorrPartiallyTrypticCharge2, @MinXCorrPartiallyTrypticCharge3
						--
						SELECT @myRowCount = @@rowcount, @myError = @@error
					End
					...
					Begin
						if @PreviewSql <> 0
							...
							exec sp_ExecuteSql @InsertSqlXTPeptideHit, @Params, @MinimumCleavageState, @MinimumDiscriminantScore, @MinimumPeptideProphetProbability, @MSGFThreshold, @MinimumPMTQualityScore, @MinDelCn2, @MinXCorrFullyTrypticCharge1, @MinXCorrFullyTrypticCharge2, @MinXCorrFullyTrypticCharge3, @MinXCorrPartiallyTrypticCharge1, @MinXCorrPartiallyTrypticCharge2, @MinXCorrPartiallyTrypticCharge3
						--
						SELECT @myRowCount = @@rowcount, @myError = @@error
					End
					...
					Begin
						if @PreviewSql <> 0
							...
							exec sp_ExecuteSql @InsertSqlInsPeptideHit, @Params, @MinimumCleavageState, @MinimumDiscriminantScore, @MinimumPeptideProphetProbability, @MSGFThreshold, @MinimumPMTQualityScore, @MinDelCn2, @MinXCorrFullyTrypticCharge1, @MinXCorrFullyTrypticCharge2, @MinXCorrFullyTrypticCharge3, @MinXCorrPartiallyTrypticCharge1, @MinXCorrPartiallyTrypticCharge2, @MinXCorrPartiallyTrypticCharge3
						--
						SELECT @myRowCount = @@rowcount, @myError = @@error
					End
					...
					Begin
						if @PreviewSql <> 0
							...
							exec sp_ExecuteSql @InsertSqlMsgPeptideHit, @Params, @MinimumCleavageState, @MinimumDiscriminantScore, @MinimumPeptideProphetProbability, @MSGFThreshold, @MinimumPMTQualityScore, @MinDelCn2, @MinXCorrFullyTrypticCharge1, @MinXCorrFullyTrypticCharge2, @MinXCorrFullyTrypticCharge3, @MinXCorrPartiallyTrypticCharge1, @MinXCorrPartiallyTrypticCharge2, @MinXCorrPartiallyTrypticCharge3
						--
						SELECT @myRowCount = @@rowcount, @myError = @@error
					End
					...
					Begin
						if @PreviewSql <> 0
							...
							exec sp_ExecuteSql @InsertSqlMsaPeptideHit, @Params, @MinimumCleavageState, @MinimumDiscriminantScore, @MinimumPeptideProphetProbability, @MSGFThreshold, @MinimumPMTQualityScore, @MinDelCn2, @MinXCorrFullyTrypticCharge1, @MinXCorrFullyTrypticCharge2, @MinXCorrFullyTrypticCharge3, @MinXCorrPartiallyTrypticCharge1, @MinXCorrPartiallyTrypticCharge2, @MinXCorrPartiallyTrypticCharge3
						--
						SELECT @myRowCount = @@rowcount, @myError = @@error
					End
		...
		Begin
			...
			If @ReturnMTTable <> 0 OR @ReturnProteinTable <> 0 Or @ReturnProteinMapTable <> 0
			Begin
				-- Return the AMT tag and/or protein details
				-- Need to first populate #Tmp_MTs_ToExport
			...
			Begin
				SELECT TAD.Job, Dataset, Dataset_ID, 
					Dataset_Created_DMS, 
					...
					Parameter_File_Name, Settings_File_Name, Organism_DB_Name, 
					Protein_Collection_List, Protein_Options_List, 
					Vol_Client + Dataset_Folder AS Storage_Path_Archive, 
					Vol_Server + Dataset_Folder AS Storage_Path_Local, 
					Results_Folder, Completed, TAD.ResultType, 
					Separation_Sys_Type, PreDigest_Internal_Std, 
					PostDigest_Internal_Std, Dataset_Internal_Std, 
					Enzyme_ID, Labelling, 
					Created_PMT_Tag_DB AS Job_Created_DB, PMTs_Last_Affected, 
	...
	Begin Catch
		...
		Set @CallingProcName = IsNull(ERROR_PROCEDURE(), 'PMExportPeptidesForJobs')
...
GO
GRANT VIEW DEFINITION ON [dbo].[PMExportPeptidesForJobs] TO [MTS_DB_Dev] AS [dbo]
GO
GRANT VIEW DEFINITION ON [dbo].[PMExportPeptidesForJobs] TO [MTS_DB_Lite] AS [dbo]
GO
