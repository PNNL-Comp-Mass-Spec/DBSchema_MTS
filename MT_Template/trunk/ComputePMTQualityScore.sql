...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[ComputePMTQualityScore]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[ComputePMTQualityScore]
GO
...
**	Author: mem
**  
**  Updated: 01/13/2004 mem - added @ResetScoresToZero parameter
...
As
	...
	Begin
		CREATE TABLE #PeptideStats (
			...
			[MaxDiscriminantScore] [float] NOT NULL
		) ON [PRIMARY]
	...
	-----------------------------------------------------------
	Declare @SavedDeltaCnComparison varchar(2),
			...
			Else
			 Begin -- <c>
				...
				Begin -- <d>
					...
					If @SavedDeltaCnComparison <> @DeltaCnComparison OR @SavedDeltaCnThreshold <> @DeltaCnThreshold OR
					...
					Begin -- <e>
						...
						Set @Sql = @Sql +   ' ObservationCount, Charge_State, MaxXCorr, MaxDiscriminantScore	'		
						Set @Sql = @Sql + ' )'
						...
						Set @Sql = @Sql +   ' MAX(StatsQ.MaxXCorr) AS MaxXCorr,'
						Set @Sql = @Sql +   ' MAX(StatsQ.MaxDiscriminantScore) AS MaxDiscriminantScore'
						Set @Sql = @Sql + ' FROM ('
					...
					Set @Sql = @sql +          ' MaxDiscriminantScore ' + @DiscriminantScoreComparison +      Convert(varchar(11), @DiscriminantScoreThreshold) + ' AND '
