...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[AddDefaultPeakMatchingTasks]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[AddDefaultPeakMatchingTasks]
GO
...
**		Date: 6/24/2003
**
**		Updated: 7/22/2003 mem
**		Updated: 7/30/2003 mem
**				 9/11/2003 mem - added new parameters
...
*****************************************************/
	...
	@JobListFilter varchar(1024) = '',			-- Optional parameter: a comma separated list of Job Numbers; will only add the given jobs numbers
...
AS
	...
	If Len(IsNull(@JobListFilter, '')) > 0
	  Begin
		...
		Set @Sql = @Sql + ' INSERT INTO #XAJ'
		Set @Sql = @Sql + ' SELECT	T.Job, '
		Set @Sql = @Sql + ' 		T.Instrument, '
		Set @Sql = @Sql + ' 		T.Dataset, IsNull(T.Labelling, '''')'
		Set @Sql = @Sql + ' FROM	dbo.T_FTICR_Analysis_Description AS T'
		Set @Sql = @Sql + ' WHERE T.Job IN (' + @JobListFilter + ') AND '
		Set @Sql = @Sql + ' 	(NOT (T.Analysis_Tool LIKE ''%TIC%''))'
	...
	Else
	  Begin
		...
		FROM	dbo.T_FTICR_Analysis_Description AS T
		WHERE	(NOT EXISTS
					(	SELECT *
	...
	Begin
		...
		If @@rowcount = 0 OR @job = 0
			Begin
				-- Set up exit from loop If no more jobs to do
				--
				Set @done = 1
			End
		Else
