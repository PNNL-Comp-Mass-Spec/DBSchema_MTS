...
AS
	...
	Set @Mass_Tag_ID = -100000000
	Declare @peptide varchar(850)
	Set @peptide = ''
	Declare @orig_peptide varchar(850)  --same as peptide, but doesn't get truncated
	...
	*/
	...
	-----------------------------------------------
	--Loop through each entry in the T_Mass_Tags table, computing monoisotopic mass if possible
	-----------------------------------------------
	While @continue > 0
	Begin
		If @RecomputeAll <> 0
		...
		Else
			...
			SELECT TOP 1
					...
					@modDescription = Mod_Description
			FROM T_Mass_Tags 
			WHERE Mass_Tag_ID > @Mass_Tag_ID AND Monoisotopic_Mass IS Null
			ORDER BY Mass_Tag_ID ASC
		...
		Begin
			...
			If @myRowCount = 0
			Begin
				Set @message = 'Unknown symbol ' + @currentAA + ' for Mass_Tag_ID ' + Convert(varchar(11), @Mass_Tag_ID) + ', ' + @orig_peptide
		...
		Begin
			...
			if @result <> 0
			begin
				Set @message = 'Error unrolling mod description ' + @modDescription + ' for Mass_Tag_ID ' + Convert(varchar(11), @Mass_Tag_ID)
			...
			Begin
				Set @message = 'No mods found in ' + @modDescription + ' for Mass_Tag_ID ' + Convert(varchar(11), @Mass_Tag_ID) + ' although Mod_Count in T_Mass_Tags is ' + Convert(varchar(11), @modCount)
				Execute PostLogEntry 'Error', @message, 'CalculateMonoisotopicMass'
			...
			Begin
				...
				WHERE	#TModDescriptors.Mass_Tag_ID = @Mass_Tag_ID
				--
				SELECT @modCountValid = @@rowcount, @myError = @@error
				--
				If @myError <> 0
				Begin
					Set @message = 'Error updating the mass to include modification masses for Mass_Tag_ID ' + Convert(varchar(11), @Mass_Tag_ID) + ', ' + @orig_peptide
					Goto done
				End

				If @modCountValid = 0
				Begin
					Set @message = 'No valid mods found in ' + @modDescription + ' for Mass_Tag_ID ' + Convert(varchar(11), @Mass_Tag_ID) + ' although Mod_Count in T_Mass_Tags is ' + Convert(varchar(11), @modCount)
					Execute PostLogEntry 'Error', @message, 'CalculateMonoisotopicMass'
				...
				Begin
					...
					Begin
						Set @message = 'Found ' + Convert(varchar(11), @modCountValid) + ' valid mods in ' + @modDescription + ' for Mass_Tag_ID ' + Convert(varchar(11), @Mass_Tag_ID) + ' although Mod_Count in T_Mass_Tags is ' + Convert(varchar(11), @modCount)
						Execute PostLogEntry 'Error', @message, 'CalculateMonoisotopicMass'
