...
**		Date: 4/3/2002
**
**		Updated: 07/05/2004 mem - Added @JobToExport parameter and modified
...
As
	...
	Set @pepFilePath = '"' + @outFileFolderPath + 'temp_' + Convert(varchar(64), NewID()) + '_' + @outFileName + '"'
	...
	Set @outFilePath = '"' + @outFileFolderPath + @outFileName + '"'
	...
	Set @S = @S +   ' CONVERT(real, Pep.MH) AS MH, SS.XCorr, SS.DeltaCn,'
	...
	Set @S = @S +   ' T_Peptides Pep ON Pep.Analysis_ID = TJM.Job INNER JOIN'
	Set @S = @S +   ' T_Score_Sequest SS ON Pep.Peptide_ID = SS.Peptide_ID INNER JOIN'
	Set @S = @S +   ' T_Sequence Seq ON Pep.Seq_ID = Seq.Seq_ID'
	Set @S = @S + ' WHERE TJM.Task_ID = ' + Convert(varchar(9), @TaskID)
	Set @S = @S + ' ORDER BY Pep.Analysis_ID, Pep.Scan_Number, Pep.Charge_State, SS.XCorr DESC, Pep.Seq_ID'
	--
	Exec (@S)
	--
	...
	Set @cmd = 'bcp "' + @S + '" queryout ' + @pepFilePath + ' -c -T'
	--
	EXEC @result = master..xp_cmdshell @cmd, NO_OUTPUT 
	--
	SELECT @myRowCount = @@rowcount, @myError = @@error
