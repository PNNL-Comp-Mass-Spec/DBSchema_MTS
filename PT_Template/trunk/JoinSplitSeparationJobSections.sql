...
As
	...
	SELECT	@JoinedDatasetID as DatasetID, 
			@JoinedMASICJobNum as Masic_Job, 
			@JoinedSequestJobNum as Sequest_Job,  
	...
	SET Process_State = 7
	FROM T_Analysis_Description
	WHERE (Dataset LIKE @DatasetMatch) AND (Job < @MinJoinedJobValue) And Process_State <> 7
	--
	SELECT @myRowCount = @@rowcount, @myError = @@error
	--
	If @myError <> 0
	...
	*/
	...
	SELECT @JoinedMASICJobNum AS Job, 
		@JoinedDataset AS Dataset, 
		@JoinedDatasetID AS DatasetID, Experiment, Campaign, Organism, 
		Instrument_Class, Instrument, Analysis_Tool, 
		Parameter_File_Name, Settings_File_Name, 
		Organism_DB_Name, Protein_Collection_List, Protein_Options_List, 
		'Virtual' AS Vol_Client, 'Virtual' AS Vol_Server, 'Virtual' AS Storage_Path, 
		'Virtual' AS Dataset_Folder, 'Virtual' AS Results_Folder, 
		GETDATE() AS Completed, ResultType, Separation_Sys_Type, 
		PreDigest_Internal_Std, PostDigest_Internal_Std, 
		Dataset_Internal_Std, Enzyme_ID, Labelling, GETDATE() 
		AS Created, GETDATE() AS Last_Affected, 
		6 AS Process_State
	FROM T_Analysis_Description
	WHERE (Dataset LIKE @DatasetMatch) AND 
		(Analysis_Tool = 'masic_finnigan') AND (Job < @MinJoinedJobValue)
	GROUP BY Experiment, Campaign, Organism, Instrument_Class, 
		Instrument, Analysis_Tool, Parameter_File_Name, 
		Settings_File_Name, Organism_DB_Name, Protein_Collection_List, Protein_Options_List, 
		ResultType, Separation_Sys_Type, PreDigest_Internal_Std, 
		PostDigest_Internal_Std, Dataset_Internal_Std, Enzyme_ID, 
		Labelling
	--
	SELECT @myRowCount = @@rowcount, @myError = @@error
	...
	FROM T_Analysis_Description
	WHERE (Dataset LIKE @DatasetMatch) AND 
		(Analysis_Tool = 'Sequest') AND (Job < @MinJoinedJobValue)
	GROUP BY Experiment, Campaign, Organism, Instrument_Class, 
		Instrument, Analysis_Tool, Parameter_File_Name, 
		Settings_File_Name, Organism_DB_Name, Protein_Collection_List, Protein_Options_List, 
		ResultType, Separation_Sys_Type, PreDigest_Internal_Std, 
	...
	*/
	...
	SELECT @JoinedMASICJobNum AS Joined_Job_ID, Job, CONVERT(int, SUBSTRING(Dataset, PATINDEX(@SectionNumberPatternMatch, Dataset) + @SectionNumberPatternMatchOffset, @SectionNumberExtractLength)) AS Section
	FROM T_Analysis_Description
	WHERE (Dataset LIKE @DatasetMatch) AND 
		  (Analysis_Tool = 'MASIC_Finnigan') AND (Job < @MinJoinedJobValue)
	ORDER BY CONVERT(int, SUBSTRING(Dataset, PATINDEX(@SectionNumberPatternMatch, Dataset) + @SectionNumberPatternMatchOffset, @SectionNumberExtractLength))
	--
	SELECT @myRowCount = @@rowcount, @myError = @@error
	--
	If @myError <> 0
	...
	INSERT INTO T_Joined_Job_Details
		(Joined_Job_ID, Source_Job, Section)
	SELECT @JoinedSequestJobNum AS Joined_Job_ID, Job, CONVERT(int, SUBSTRING(Dataset, PATINDEX(@SectionNumberPatternMatch, Dataset) + @SectionNumberPatternMatchOffset, @SectionNumberExtractLength)) AS Section
	FROM T_Analysis_Description
	WHERE (Dataset LIKE @DatasetMatch) AND 
		  (Analysis_Tool = 'Sequest') AND (Job < @MinJoinedJobValue)
	ORDER BY CONVERT(int, SUBSTRING(Dataset, PATINDEX(@SectionNumberPatternMatch, Dataset) + @SectionNumberPatternMatchOffset, @SectionNumberExtractLength))
	...
	*/
	...
	FROM (SELECT JJD.Joined_Job_ID, JJD.Section, DSS.Job, 
		...
		FROM T_Joined_Job_Details JJD INNER JOIN
			T_Analysis_Description TAD_JoinedJob ON 
			JJD.Joined_Job_ID = TAD_JoinedJob.Job INNER JOIN
			T_Dataset_Stats_Scans DSS ON 
			JJD.Source_Job = DSS.Job
	...
	FROM (SELECT JJD.Joined_Job_ID, JJD.Section, 
		...
		FROM T_Joined_Job_Details JJD INNER JOIN
			T_Analysis_Description TAD_JoinedJob ON 
			JJD.Joined_Job_ID = TAD_JoinedJob.Job INNER
			JOIN
			T_Analysis_Description TAD ON 
			JJD.Source_Job = TAD.Job INNER JOIN
			T_Peptides P ON TAD.Job = P.Analysis_ID
		WHERE JJD.Joined_Job_Id = @JoinedSequestJobNum
		GROUP BY JJD.Joined_Job_ID, JJD.Section, TAD.Job, 
			TAD.Dataset) LookupQ INNER JOIN
		T_Joined_Job_Details ON 
		LookupQ.Joined_Job_ID = T_Joined_Job_Details.Joined_Job_ID
		AND 
		LookupQ.Source_Job = T_Joined_Job_Details.Source_Job
	--
	SELECT @myRowCount = @@rowcount, @myError = @@error
	--
	If @myError <> 0
	...
	FROM (SELECT JJD.Joined_Job_ID, JJD.Source_Job, JJD.Section, 
			JJDNext.Section AS SectionNext, CONVERT(real, 
			DSNext.Acq_Time_Start - DS.Acq_Time_End) 
			* 24 * 60 AS Gap_Minutes
		FROM T_Datasets DS INNER JOIN
			T_Joined_Job_Details JJD INNER JOIN
			T_Analysis_Description TAD ON 
			JJD.Source_Job = TAD.Job ON 
			DS.Dataset_ID = TAD.Dataset_ID INNER JOIN
			T_Datasets DSNext INNER JOIN
			T_Analysis_Description TADNext ON 
			DSNext.Dataset_ID = TADNext.Dataset_ID INNER JOIN
			T_Joined_Job_Details JJDNext ON 
			TADNext.Job = JJDNext.Source_Job ON 
			JJD.Joined_Job_ID = JJDNext.Joined_Job_ID AND 
	...
	FROM (SELECT JJD.Joined_Job_ID, JJD.Source_Job, 
			JJD.Section, SUM(TotalsQ.Total_Scan_Count) 
			AS Scan_Number_Added, 
			SUM(TotalsQ.Total_Run_Time) 
			AS Scan_Time_Added
		FROM T_Joined_Job_Details JJD INNER JOIN
				(SELECT Joined_Job_ID, Source_Job, Section, 
					Scan_Number_End - Scan_Number_Start + 1 AS Total_Scan_Count,
					Scan_Time_End + gap_to_Next_section_Minutes
					AS Total_Run_Time
				FROM T_Joined_Job_Details JJD
				WHERE Joined_Job_Id = @JoinedMASICJobNum) TotalsQ ON 
			JJD.Joined_Job_ID = TotalsQ.Joined_Job_ID AND
			JJD.Section > TotalsQ.Section
		GROUP BY JJD.Joined_Job_ID, JJD.Source_Job, 
			JJD.Section) LookupQ INNER JOIN
		T_Joined_Job_Details JJD ON 
		LookupQ.Joined_Job_ID = JJD.Joined_Job_ID AND 
	...
	*/
	...
	FROM T_Seq_Candidates SC INNER JOIN
		T_Joined_Job_Details JJD ON SC.Job = JJD.Source_Job
	WHERE JJD.Joined_Job_ID = @JoinedSequestJobNum
	--
	SELECT @myRowCount = @@rowcount, @myError = @@error
	--
	Set @task = 'changing Job to Joined Job ID in T_Seq_Candidates using T_Joined_Job_Details'
	If @myError <> 0
	Begin
		Set @message = 'Error ' + @Task
		Goto Done
