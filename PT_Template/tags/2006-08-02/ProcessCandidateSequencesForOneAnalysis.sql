...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[ProcessCandidateSequencesForOneAnalysis]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[ProcessCandidateSequencesForOneAnalysis]
GO
...
As
	...
	declare @CandidatesSequencesTableName varchar(256)
	...
	Begin
		...
		Set @CandidatesSequencesTableName = @@ServerName + '.[' + DB_Name() + '].dbo.T_Seq_Candidates'
	...
	Begin
		...
		-----------------------------------------------------------
		-- Populate @CandidatesSequencesTableName with the candidate sequences
		-----------------------------------------------------------
		--
		declare @Sql varchar(1024)
		Set @Sql = ''
		Set @Sql = @Sql + ' INSERT INTO ' + @MasterSequencesServerName + '.' + @CandidatesSequencesTableName
	...
	begin
		If Len(@message) = 0
			set @message = 'Error calling Master_Sequences.dbo.ProcessCandidateSequences for job ' + @jobStr + ': ' + convert(varchar(12), @myError)
		else
			set @message = 'Error calling Master_Sequences.dbo.ProcessCandidateSequences for job ' + @jobStr + ': ' + @message
	...
	Begin
		...
		Set @Sql = @Sql +  ' ' + @MasterSequencesServerName + '.' + @CandidatesSequencesTableName + ' MSeqData '
	...
	-- ToDo: Possibly start a transaction here and end it after 
	-- Inserting new sequences into T_Sequence
