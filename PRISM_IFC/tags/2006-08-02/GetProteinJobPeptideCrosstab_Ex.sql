...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[GetProteinJobPeptideCrosstab_Ex]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[GetProteinJobPeptideCrosstab_Ex]
GO
...
*****************************************************/
	...
	@bait varchar(128) = 'SO1209',
	@mode varchar(32) = 'Crosstab_Report', -- 'Crosstab_Report', 'Interaction_Report', 'Interaction_Rollup_Report', 'Preview_Data_Analysis_Jobs', 'Preview_Experiments', 'Preview_Proteins'
...
AS
	...
	if @myError <> 0
	begin
		set @message = 'Could not populate temporary table for observed proteins'
		goto Done
	end
...
/**/
	...
	if @bait <> ''
		begin
			set @sql = @sql + '''' + @bait + ''' as Bait, ' 
			set @sql = @sql + 'Reference as Interactor,  ' 
			set @sql = @sql + 'Convert(decimal(8,0), Monoisotopic_Mass / 1000) as [Mass (KDa)],  ' 
			set @sql = @sql + 'Reference_Notes,  ' 
		end
	else
		begin
			set @sql = @sql + 'S, ' 
			set @sql = @sql + 'Reference,  ' 
			set @sql = @sql + 'Monoisotopic_Mass,  ' 
			set @sql = @sql + 'Reference_Notes,  ' 
			set @sql = @sql + 'Standard,  ' 
		end
