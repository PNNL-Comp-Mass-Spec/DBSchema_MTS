...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[GetProteinsIdentified_Old]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[GetProteinsIdentified_Old]
GO
...
**			  10/27/2004 mem - Fixed computation of Mass_Tag_Count and changed the Dataset_Count and Job_Count columns to Dataset_Count_Avg and Job_Count_Avg
**    
*****************************************************/
...
As
	...
	If CharIndex('MSMS_High_Normalized_Score_Avg', @outputColumnNameList) > 0 OR CharIndex('MSMS_High_Normalized_Score_Avg', @criteriaSql) > 0
		Set @sqlSelect = @sqlSelect + ', AVG(MT.High_Normalized_Score) AS MSMS_High_Normalized_Score_Avg'
