/****** Object:  StoredProcedure [dbo].[MakeProvisionalMTDB] ******/
SET ANSI_NULLS ON
GO
...
**  new mass tag database and returns the name and ID
**
...
AS
	...
	set @seqNum = 0
	--
	SELECT     TOP 1 @newDBID = MT_DB_ID + 1, @seqNum = MT_DB_ID - 991
	FROM         T_MTS_MT_DBs WITH (HOLDLOCK)
	...
	set @newDBName = 'MT_' + @newDBNameRoot + '_' + @newDBNameType + cast(@seqNum as varchar(12))

	---------------------------------------------------
	-- Create provisional entry in MT database list
	---------------------------------------------------
		
	INSERT INTO T_MTS_MT_DBs (
		MT_DB_ID, 
		MT_DB_Name, 
		Server_ID, 
		State_ID
