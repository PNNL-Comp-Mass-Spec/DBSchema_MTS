...
SELECT TOP 100 PERCENT Pep.Analysis_ID, Pep.Scan_Number, 
    ...
    AS MH, SS.XCorr AS Normalized_Score, SS.DeltaCn, 
    CONVERT(real, SS.Sp) AS Sp, Seq.Cleavage_State_Max, 
    Pep.Scan_Time_Peak_Apex
FROM dbo.T_Analysis_Description TAD INNER JOIN
    dbo.T_Peptides Pep INNER JOIN
    dbo.T_Sequence Seq ON Pep.Seq_ID = Seq.Seq_ID ON 
    TAD.Job = Pep.Analysis_ID LEFT OUTER JOIN
...
SELECT TOP 100 PERCENT Pep.Analysis_ID, Pep.Scan_Number, 
    ...
    = 0 THEN 'none' ELSE Seq.Mod_Description END AS Mod_Description,
     Pep.Seq_ID, Pep.Charge_State, CONVERT(real, Pep.MH) 
    AS MH, X.Normalized_Score AS Normalized_Score, 
    0 AS DeltaCn, 500 AS SP, Seq.Cleavage_State_Max, 
    Pep.Scan_Time_Peak_Apex
FROM T_Analysis_Description TAD INNER JOIN
    T_Peptides Pep INNER JOIN
    T_Sequence Seq ON Pep.Seq_ID = Seq.Seq_ID ON 
    TAD.Job = Pep.Analysis_ID INNER JOIN
...
ORDER BY Pep.Analysis_ID, Pep.Scan_Number, Pep.Charge_State, 
    Normalized_Score DESC, Pep.Seq_ID
