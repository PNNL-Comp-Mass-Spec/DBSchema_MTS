...
/****************************************************
**
**	Desc:	Updates the data in T_DMS_Filter_Set_Overview_Cached and T_DMS_Filter_Set_Details_Cached using DMS
**
...
AS
	...
	Begin Try
		...
		USING (SELECT Filter_Set_Criteria_ID, Filter_Set_ID, Filter_Criteria_Group_ID,
					...
					Criterion_Comparison, Criterion_Value
		...
		WHEN Matched AND (target.Filter_Set_ID <> source.Filter_Set_ID OR
						...
						target.Criterion_Name <> source.Criterion_Name OR
			...
			UPDATE set Filter_Set_ID = source.Filter_Set_ID,
					...
					Criterion_Name = source.Criterion_Name,
		...
		WHEN Not Matched THEN
			...
			VALUES (source.Filter_Set_Criteria_ID, source.Filter_Set_ID, source.Filter_Criteria_Group_ID,
					source.Criterion_ID, source.Criterion_Name,
					source.Criterion_Comparison, source.Criterion_Value,
					GetDate())
		WHEN NOT MATCHED BY SOURCE THEN 
			DELETE
		OUTPUT $action INTO #Tmp_UpdateSummary
