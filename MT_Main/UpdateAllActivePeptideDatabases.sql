...
As	
	...
	Begin Try -- <x>
		...
		-----------------------------------------------------------
		Set @CurrentLocation = 'Delete old Peptide DB entries in the current activity table'
		...
		-----------------------------------------------------------
		Set @CurrentLocation = 'Delete old Peptide DB entries in the current activity history table'
		...
		-----------------------------------------------------------
		-- 
		Set @CurrentLocation = 'Look for peptide databases that were created over 36 hours ago'
		...
		-----------------------------------------------------------
		--
		Set @CurrentLocation = 'Call PreviewCurrentActivityForPeptideDBs and PreviewCurrentActivityForMTDBs'
		...
		BEGIN -- <a>
			...
			-- Get next entry
			Set @CurrentLocation = 'Get next available entry from peptide database list table'
			...
			Begin -- <b>
				...
				If @demandImport > 0 AND @PDB_State >= 15
				Begin
					set @SkipUpdate = 1

					set @message = 'Skipping update for database ' + @PDB_Name + ' since its state is ' + Convert(varchar(12), @PDB_State) + '; change MTL_Demand_Import to 0 for this DB to avoid this error message'
				...
				Begin -- <c>
					Begin Try -- <y>
						Set @CurrentLocation = 'Update ' + @PDB_Name
						...
						--
						Set @CurrentLocation = 'Call GetDBSchemaVersionByDBName for ' + @PDB_Name
						...
						Set @CurrentLocation = 'Call GetStatisticsFromExternalDB for ' + @PDB_Name
						...
						/*				
						** DB Schema Version 1 is no longer supported
						**
							if @importNeeded > 0
							begin
						...
						*/
						...
						Begin -- <c>
							...
							Begin
								...
								-----------------------------------------------------------	
								Set @CurrentLocation = 'Call MasterUpdateProcessImport in ' + @PDB_Name
							...
							Set @CurrentLocation = 'Call MasterUpdateProcessBackground in ' + @PDB_Name
						...
						Set @CurrentLocation = 'Record update complete for ' + @PDB_Name
						...
						Set @CurrentLocation = 'Log end of update for ' + @PDB_Name
						--
					...
					Begin Catch
						...
						Set @CallingProcName = IsNull(ERROR_PROCEDURE(), 'UpdateAllActivePeptideDatabases')
			...
			Set @CurrentLocation = 'Call VerifyUpdateEnabled for Peptide_DB_Update'
...
GO
GRANT VIEW DEFINITION ON [dbo].[UpdateAllActivePeptideDatabases] TO [MTS_DB_Dev] AS [dbo]
GO
GRANT VIEW DEFINITION ON [dbo].[UpdateAllActivePeptideDatabases] TO [MTS_DB_Lite] AS [dbo]
GO
