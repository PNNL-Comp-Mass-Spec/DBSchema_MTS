...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[QueryAllMassTagDatabasesForMTStats]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[QueryAllMassTagDatabasesForMTStats]
GO
...
As
	...
	Set @Sql = @Sql + ' WHERE MTL_State IN (1,2,5) AND NOT ' + @SkipDBWhereClause
	Exec (@Sql)
	...
	Begin --<a>
		...
		Begin
			...
			Begin
				...
				Set @Sql = @Sql + ' FROM [' + @MTL_Name + ']..T_Mass_Tags MT INNER JOIN'
				Set @Sql = @Sql +   ' [' + @MTL_Name + ']..T_Mass_Tags_NET MTN ON MT.Mass_Tag_ID = MTN.Mass_Tag_ID INNER JOIN'
				Set @Sql = @Sql + ' [' + @MTL_Name + ']..T_Mass_Tag_to_Protein_Map MTPM ON MT.Mass_Tag_ID = MTPM.Mass_Tag_ID'
				Set @Sql = @Sql + ' WHERE MT.High_Discriminant_Score >= ' + Convert(varchar(9), @MinimumDiscriminantScore) + ' AND NOT MTN.Avg_GANET Is Null'
				Set @Sql = @Sql + ' GROUP BY MT.Mass_Tag_ID, MT.Monoisotopic_Mass, MT.High_Discriminant_Score, MTN.Avg_GANET, '
				Set @Sql = @Sql +   ' MTN.StD_GANET, MTN.Cnt_GANET, MTN.PNET'
