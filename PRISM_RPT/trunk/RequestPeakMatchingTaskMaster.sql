...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[RequestPeakMatchingTaskMaster]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[RequestPeakMatchingTaskMaster]
GO
...
**		Can limit which priority tasks to return using
**		@PriorityMin and @PriorityMax
**
**		If a task is returned to the calling computer, then 
**		updates T_Peak_Matching_Activity with the relevant information
**
**		Auth: mem
**		Date: 6/19/2003
**
...
As
	...
	-- Uncomment the following to disable automated peak matching
	-- return 0
	...
	--Exec CheckStalledPeakMatchingProcessors 120
						...
						If @TaskAvailable = 1 And @myError = 0
							Begin
								...
								INSERT INTO T_Peak_Matching_History (
												PM_AssignedProcessorName, Server_Name, 
												MTDBName, TaskID, Job, 
												PM_Start, Output_Folder_Path)
								VALUES (@ProcessorName, @serverName,
										@mtdbName, @TaskID, @analysisJob,
								...
								If @ActivityRowCount = 0 AND @myError = 0
									INSERT INTO T_Peak_Matching_Activity (PM_AssignedProcessorName)
								...
								Begin
									...
									SET Working = 1, Server_Name = @serverName, MTDBName = @mtdbName, 
