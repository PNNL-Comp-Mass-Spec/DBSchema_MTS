...
SET @DatabaseRoleName = 'dms_sp_user'	-- 'role_name_goes_here'
...
FROM [dbo].[sysusers]
WHERE [name] = @DatabaseRoleName AND
      ([issqlrole] = 1 OR
       [isapprole] = 1) 
      AND
[name] NOT IN ('public', 'INFORMATION_SCHEMA', 'db_owner', 'db_accessadmin', 'db_securityadmin', 
...
BEGIN
	...
	ELSE
		SET @errStatement =
			'Role ' + @DatabaseRoleName + ' does not exist in ' + DB_NAME() + '.' + CHAR(13) +
...
BEGIN
	SET @msgStatement =
		'--Security creation script for role ' + @DatabaseRoleName + CHAR(13) +
