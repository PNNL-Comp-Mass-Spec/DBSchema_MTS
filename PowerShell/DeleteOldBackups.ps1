...
# (customize the number of .bak files using -recentBakToKeep)
#
...
# DeleteOldBackups.ps1 -folder \\MyServer\DBBackups\ -daysToKeep 90 -recentBakToKeep 2 -depth 1 -msgLevel 1
...
# Added -recentBakToKeep in April 2017
#
...
function Delete-DBBackupFiles {
    ...
    foreach($x in $bakFiles)
    {
            ...
            if ($item -le $recentBakToKeep) {
                # Always keep the $recentBakToKeep newest .bak files (and their corresponding .trn files)
